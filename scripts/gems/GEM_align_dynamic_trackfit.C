...
void CHI2_FCN( int &npar, double *gin, double &f, double *par, int flag){
  ...
  for( int itr=0; itr<NTRACKS; itr++ ){
    ...
    //we also need to dynamically update the track fit! 
    // Ignore the TRACK parameters recalculate them! 
    double xtr,ytr,xptr,yptr;
    ...
    for( int ihit=0; ihit<TRACKNHITS[itr]; ihit++ ){
      ...
      sum_x += hitpos_global.X();
      sum_y += hitpos_global.Y();
      sum_z += hitpos_global.Z();
      sum_xz += hitpos_global.X()*hitpos_global.Z();
      sum_yz += hitpos_global.Y()*hitpos_global.Z();
      sum_zz += hitpos_global.Z()*hitpos_global.Z();
    ...
    double denom = TRACKNHITS[itr]*sum_zz - sum_z*sum_z;
    xtr = (sum_zz*sum_x - sum_z * sum_xz)/denom;
    ytr = (sum_zz*sum_y - sum_z * sum_yz)/denom;
    xptr = (TRACKNHITS[itr]*sum_xz - sum_x*sum_z)/denom;
    yptr = (TRACKNHITS[itr]*sum_yz - sum_y*sum_z)/denom;

    //loop over all the hits again and calculate chi2 contributions based on updated track parameters:
    for( int ihit=0; ihit<TRACKNHITS[itr]; ihit++ ){
      double ulocal = HITX[itr][ihit];
      double vlocal = HITY[itr][ihit];
      ...
      TVector3 trackpos_global( xtr + xptr * hitpos_global.Z(),
				ytr + yptr * hitpos_global.Z(),
				hitpos_global.Z() );
...
void GEM_align_dynamic_trackfit( const char *configfilename, const char *outputfilename="newGEMalignment.txt" ){
