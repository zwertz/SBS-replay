...
void PiPlusNeutronDownbending( const char *configfilename, const char *outputfilename="ElasticTemp.root" ){
  ...
  Tout->Branch( "Egamma", &T_Egamma_recon, "Egamma/D");

  double T_thetapi_expect_endpoint;
  Tout->Branch("thetapi_expect", &T_thetapi_expect_endpoint, "thetapi_expect/D");
  ...
  while( C->GetEntry(ievent) ){
    ...
    if( int(ntrack) >= 1 && passed_global_cut ){
      ...
      Ebeam_corrected = ebeam; //don't correct for eloss.
      ...
      //Now what about the formula for ppi (expected) when 
      ...
      double precon = p[0];
      double pitheta = etheta;

      double Epi = sqrt(pow(precon,2) + pow(Mpi,2) );

      double thetapi_expect_endpoint = acos(std::max(-1.,std::min(1., (2.*ebeam*(Epi-Mp) - pow(Mp,2)-pow(Mpi,2)+pow(Mn,2)+2.*Mp*Epi)/(2.*ebeam*precon) )));

      T_thetapi_expect_endpoint = thetapi_expect_endpoint;
      
      //Now reconstruct Egamma:

      double Egamma_recon = (pow( Mp, 2 ) + pow( Mpi, 2 ) - pow( Mn, 2 ) - 2.*Mp * Epi )/(2.0*(Epi - precon*cos(pitheta)-Mp));

      T_Egamma_recon = Egamma_recon;
      
      TLorentzVector Pgamma( 0, 0, Egamma_recon, Egamma_recon );
      TLorentzVector Ppion( px[0], py[0], pz[0], Epi );
      ...
      TLorentzVector Pneutron = Ptarg + Pgamma - Ppion;
      
      T_Q2_4vect = -q.M2();
      T_W2_4vect = (Ptarg + q).M2();

      TVector3 qvect = Pneutron.Vect();
      ...
      double ptheta_expect = Pneutron.Theta();
