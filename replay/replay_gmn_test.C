...
void replay_gmn_test(UInt_t runnum=10491, Long_t nevents=-1, Long_t firstevent=0, const char *fname_prefix="e1209019", UInt_t firstsegment=0, UInt_t maxsegments=1, Int_t pedestalmode=0)
{
  ...
  gHaPhysics->Add( new THaPrimaryKine( "e.kine", "electron kinematics", "bb", 0.0, 0.938 ));
  
  THaScalerEvtHandler *lScaler = new THaScalerEvtHandler("Left","HA scaler event type 140");
  lScaler->SetEvtType(140);
  gHaEvtHandlers->Add (lScaler);
  //gHaEvtHandlers->Add (new THaScalerEvtHandler("SBS","HA scaler event type 141"));
  gHaEvtHandlers->Add (new THaEpicsEvtHandler("EPICS","HA scaler event type 131"));
  ...
  TString outfilename;
  outfilename.Form( "%s/gmn_replayed_%d_testscalers.root", prefix.Data(), runnum );
  ...
  TString odef_filename = "replay_gmn_test.odef";
...
int main(int argc, char *argv[])
{
  ...
  replay_gmn_test(runnum, nevents, firstevent, 
