...
void replay_genrp_aonl1(UInt_t runnum=10491, Long_t nevents=-1, Long_t firstevent=1, const char *fname_prefix="e1217004", UInt_t firstsegment=0, UInt_t maxsegments=1, Int_t maxstream=2, Int_t pedestalmode=0, Int_t cmplots=0, Int_t usesbsgems=1)
{
  ...
  if( nevents > 0 ){
    outfilename.Form( "%s/genrp_replayed_%ld_%ldk_events.root", prefix.Data(), runnum, (nevents/1000) );
    //outfilename.Form( "%s/%s_replayed_%u_stream%d_%d_seg%u_%u_firstevent%ld_nevent%ld.root", prefix.Data(), fname_prefix, runnum, 0, maxstream, firstsegment, lastsegment, firstevent, nevents );
  } else {
    //outfilename.Form( "%s/%s_fullreplay_%u_stream%d_%d_seg%u_%u.root", prefix.Data(), fname_prefix, runnum, 0, maxstream, firstsegment, lastsegment );
    outfilename.Form( "%s/genrp_fullreplay_%ld_%ld_events.root", prefix.Data(), runnum, nevents );
  }
  ...
  //analyzer->SetSummaryFile(Form("%s/%s_%d_stream%d_%d_seg%d_%d.log", prefix.Data(), fname_prefix, runnum, 0, maxstream, firstsegment, lastsegment));
  analyzer->SetSummaryFile(Form("%s/genrp_%d_%d_nevents.log", prefix.Data(), runnum, nevents));
