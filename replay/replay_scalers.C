...
void replay_scalers(UInt_t runnum=10491, Long_t nevents=-1, Long_t firstevent=1, const char *fname_prefix="e1209016", UInt_t firstsegment=0, UInt_t maxsegments=1, Int_t maxstream=2, Int_t pedestalmode=0, Int_t cmplots=1, Int_t usesbsgems=1)
{
  ...
  if( nevents > 0 ){
    outfilename.Form( "%s/%s_scaler_replayed_%u_stream%d_%d_seg%u_%u_firstevent%ld_nevent%ld.root", prefix.Data(), fname_prefix, runnum,
		      0, maxstream, firstsegment, lastsegment, firstevent, nevents );
  } else {
    outfilename.Form( "%s/%s_scaler_fullreplay_%u_stream%d_%d_seg%u_%u.root", prefix.Data(), fname_prefix, runnum,
  ...
  analyzer->SetSummaryFile(Form("%s/scaler_replay_gen_%d_stream%d_%d_seg%d_%d.log", prefix.Data(), runnum,
				0, maxstream, firstsegment, lastsegment));
